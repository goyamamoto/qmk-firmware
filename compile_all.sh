...
read -r -a keyboard_list <<<"${KEYBOARDS:-air60v2 air75v2 halo75v2 halo96v2 gem80}"
...
if [[ ${#targets[@]} -eq 0 ]]; then
  printf 'No matching build targets found for KEYBOARDS=%s LAYOUTS=%s KEYMAPS=%s\n' "${KEYBOARDS:-air60v2 air75v2 halo75v2 halo96v2 gem80}" "${LAYOUTS:-ansi iso}" "${KEYMAPS:-default via srgb ryodeushii}" >&2
  exit 1
fi
